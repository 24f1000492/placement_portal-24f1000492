...
def login_required(role):
    """Decorator that checks session role before allowing access."""
    def decorator(f):
        @wraps(f)
...
def register_routes(app):
    ...
    def admin_dashboard():
        ...
        # Application status counts for CSS chart
        app_counts = {
        ...
        app_pcts = {k: round(v / total_apps * 100) if total_apps else 0
        ...
        recent_apps      = Application.query.order_by(Application.application_date.desc()).limit(6).all()

        return render_template('admin/dashboard.html',
                               stats=stats,
                               app_counts=app_counts, app_pcts=app_pcts,
    ...
    def admin_drives():
        ...
        return render_template('admin/drives.html',
                               drives=drives, status_filter=status_filter,
                               today=date.today())
    ...
    def company_drives():
        ...
        return render_template('company/drives.html', drives=drives, company=company,
                               today=date.today())
    ...
    def download_resume(student_id):
        ...
        folder   = os.path.join(current_app.root_path, 'static', 'uploads', 'resumes')
